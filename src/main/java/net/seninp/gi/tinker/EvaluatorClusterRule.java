    ...
    for (int w : wins) {
      for (int p : PAAS) {
        for (int a : ALPHABETS) {
          ...
          ArrayList<SameLengthMotifs> refinedClassifiedMotifs = ClusterRuleFactory
              .performPruning(series, rules, thresholdLength, thresholdCom, fractionTopDist);
          ArrayList<PackedRuleRecord> packedRules = ClusterRuleFactory
              .getPackedRule(refinedClassifiedMotifs);

          // RuleOrganizer ro = new RuleOrganizer();
          // SAXPointsNumber[] pointsOccurenceInPackedRule =
          // ro.countPointNumberAfterRemoving(series,
