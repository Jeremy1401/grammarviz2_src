...
public class GrammarVizView implements Observer, ActionListener {
    ...
    // Cluster parameters related fields
    private JPanel clusterParametersPane;
    private JLabel clusterKLabel;
    private JTextField clusterKField;
    private JLabel clusterThresholdLabel;
    private JTextField clusterThresholdField;
    ...
    private void configureGUI() {
        ...
        frame.getContentPane().add(saxParametersPane, "w 70%, split");
        frame.getContentPane().add(numerosityReductionPane, "split");
        frame.getContentPane().add(discretizePane, "split");

        frame.getContentPane().add(clusterParametersPane, "grow, wrap");
    ...
    private void buildClusterParamsPane() {
        clusterParametersPane = new JPanel();
        clusterParametersPane.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createEtchedBorder(BevelBorder.LOWERED), "cluster parameteres", TitledBorder.LEFT,
                TitledBorder.CENTER, new Font(TITLE_FONT, Font.PLAIN, 10)));

        // insets: T, L, B, R.
        MigLayout clusterPaneLayout = new MigLayout("insets 3 2 2 2",
                "[][fill,grow]10[][fill,grow]10", "[]");
        clusterParametersPane.setLayout(clusterPaneLayout);

        // the k parameter
        clusterKLabel = new JLabel("cluster k:");
        clusterKField = new JTextField(String.valueOf(this.controller.getSession().k));

        clusterThresholdLabel = new JLabel("cluster threshold:");
        clusterThresholdField = new JTextField(String.valueOf(this.controller.getSession().threshold));

        clusterParametersPane.add(clusterKLabel);
        clusterParametersPane.add(clusterKField);

        clusterParametersPane.add(clusterThresholdLabel);
        clusterParametersPane.add(clusterThresholdField);
    }
    ...
    public void actionPerformed(ActionEvent arg) {
        ...
        if (LOAD_DATA.equalsIgnoreCase(command)) {
            ...
            if (this.isTimeSeriesLoaded) {
                ...
                LOGGER.info("当前程序耗时：" + (endTime - startTime) + "ns");
            } else {
            ...
            this.clusterKField.setText(String.valueOf(this.controller.getSession().k));
            this.clusterThresholdField.setText(String.valueOf(this.controller.getSession().threshold));
...
//                    double thresholdCommon = Double.parseDouble(overlapThreshold.getText());
                this.controller.getSession().k = Integer.valueOf(this.clusterKField.getText());
                this.controller.getSession().threshold = Double.valueOf(this.clusterThresholdField.getText());
                this.controller.getSession().chartData.setClusterParams(this.controller.getSession().k, this.controller.getSession().threshold);
                dataChartPane.resetChartPanel();
...
//                            thresholdCommon);
                this.controller.getSession().chartData.performRuleCluster();  //
                long endTime = System.nanoTime();
                LOGGER.info("当前程序耗时：" + (endTime - startTime) + "ns");
...
//                }
            ...
            if (null == this.controller.getSession().chartData) {
                ...
                log(Level.INFO, "the number of origin rules: " + this.controller.getSession().chartData.getRulesNumber());
                ...
                log(Level.INFO, "the number of pruned rules: " + this.controller.getSession().chartData.getRulesNumber());
