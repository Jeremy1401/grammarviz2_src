...
import rule.cluster.RuleClusters;
import rule.cluster.RuleCluster;
/**
...
public class GrammarVizChartData extends Observable implements Observer {
     ...
     * cluster the rules
     */
    private RuleClusters ruleClusters;
    private double threshold = 0.6;  // 相似度阈值
    private int k = 1;  // 聚类个数
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(GrammarVizChartData.class);
    /**
     * Constructor.
     *
    ...
    public void setClusterParams(int k, double threshold){
        this.k = k;
...
//    }
    ...
    public ArrayList<RuleInterval> getSubsequencesPositionsByClassNum(Integer clsIdx) {
        ...
        // the sub-sequences class container
        RuleCluster thisCluster = ruleClusters.getRuleClusterByIndex(clsIdx);
        ...
        for (RuleInterval interval : thisCluster.getRuleIntervals()) {
            positions.add(new RuleInterval(interval.getStart(), interval.getEnd()));
        }
    ...
    */
    ...
    /**
     * perform rule cluster
     */
    public void performRuleCluster() {
        arrPackedRuleRecords = new ArrayList<>();

        //Define data
        DataPoint[] elements = new DataPoint[grammarRules.size()-1];
        int count = 0;
        ...
        ROCKAlgorithm rock = new ROCKAlgorithm(elements, k, threshold);
        Dendrogram dnd = rock.cluster();  // 进行聚类
        ...
        dnd.print(dnd.getTopLevel());

        // 获取聚类结果，将规则聚类到ruleClusters中
        ruleClusters = new RuleClusters();
        List<Cluster> clusters = dnd.getClustersForLevel(dnd.getTopLevel());
        Iterator<Cluster> it = clusters.iterator();
        while (it.hasNext()){
            Set<DataPoint> dataPointSet = it.next().getElements();
            DataPoint e;

            RuleCluster ruleCluster = new RuleCluster();  // 构造规则簇
            Iterator var2 = dataPointSet.iterator();  // 簇内对象迭代器
            while(var2.hasNext()){
                e = (DataPoint)var2.next();
                LOGGER.info("rule label: " + e.getLabel());
                int ruleId = Integer.parseInt(e.getLabel().replace("R", ""));  // 获取规则序号
                ruleCluster.addRuleId(ruleId);
                ruleCluster.addRuleIntervals(grammarRules.get(ruleId).getRuleIntervals());
                LOGGER.info("grammarRules intervals = " + grammarRules.get(ruleId).getRuleIntervals().toString());
            }
        ...
        // get the distance between the rule pairs
        HashMap<Integer, Map<Integer, Double>> distanceDatabase =
                RuleClusterFactory.getDistanceDatabase(grammarRules);

        Map<Integer, Map<Double, ArrayList<Integer>>> distanceMap = new TreeMap<>();

        for (Integer key : distanceDatabase.keySet()) {
            double dis = 0.0D;
            Map<Double, ArrayList<Integer>> map = new TreeMap<Double, ArrayList<Integer>>();
        ...
        ruleClusters = new RuleClusters();
        ArrayList<Integer> usedRuleIdList = new ArrayList<Integer>(grammarRules.size());
        ArrayList<Integer> random = new ArrayList<Integer>();
        for (int id = 1; id < grammarRules.size(); id++) {
            random.add(ruleNumberList[id]);
        }
...
//        LOGGER.info("乱序之后： " + random.toString());
        ...
        */
        ...
        for (RuleCluster ruleCluster : ruleClusters.getRuleClusters()) {
            int classIndex = i;
            int subsequencesNumber = ruleCluster.getSubsquenceNum(grammarRules);
            int minLength = ruleCluster.getMinRuleLen();
            int maxLength = ruleCluster.getMaxRuleLen();
